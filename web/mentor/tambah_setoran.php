...
require "../config.php";
$id = $_GET['id'];
$query = "SELECT user.nama, user.gambar, user.id AS uid, mahasantri.* FROM mahasantri
INNER JOIN user ON mahasantri.mentor_id = user.id WHERE mahasantri.id = $id";
$m = mysqli_fetch_assoc(mysqli_query($connect, $query));
$uid = $m['uid'];
$query3 = "SELECT * FROM gambar WHERE user_id = $uid";
...
if (isset($_POST['simpan'])) {
    $query = "INSERT INTO setoran (mahasantri_id, tanggal, juz, halaman, nilai, keterangan) VALUES ('$id','$date', '$juz', '$hal', '$nilai', '$ket')";
...
button: 'ok',
    ...
    header("Location: mahasantri.php?id=$id");
}
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="profile-desc">
                            <div class="profile-pic">
                                <div class="count-indicator">
                                    <div class="pp rounded-circle" style="background-image: url(../img/<?= $m['gambar'] ?>);"></div>
                                </div>
                                <div class="profile-name">
                                    <h5 class="mb-0 font-weight-normal"><?= $m['nama'] ?></h5>
                                    <span>Mentor</span>
                                </div>
                            </div>
                            ...
                            <li class="nav-item dropdown">
                                <a class="nav-link" id="profileDropdown" href="#" data-toggle="dropdown">
                                    <div class="navbar-profile">
                                        <div class="pps rounded-circle" style="background-image: url(../img/<?= $m['gambar'] ?>);"></div>
                                        <p class="mb-0 d-none d-sm-block navbar-profile-name"><?= $m['nama'] ?></p>
                    ...
                    <div class="content-wrapper">
                        <div class="row ">
                            <div class="col-12 grid-margin">
                                <div class="card">
                                    <div class="card-body">
                                        <a href="mahasantri.php?id=<?= $id ?>" class="btn btn-outline-warning" style="float: right;"><i class="fa fa-arrow-left"></i> Kembali</a>
                                        <h4 class="card-title">Tambah Data Setoran <?= $m['nama_mhs'] ?></h4>
                                        ...
                                        <form class="forms-sample" method="POST">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input required type="number" class="form-control" name="juz" id="juz" placeholder="Juz">
                                            </div>
                                            <div class="form-group">
                                                <label for="hal">Halaman</label>
                                                <input required type="number" class="form-control" name="hal" id="hal" placeholder="Halaman">
                                            </div>
                                            <div class="form-group">
                                                <label for="nilai">Nilai</label>
                                                <input required type="number" class="form-control" name="nilai" id="nilai" placeholder="Nilai">
                                            </div>
                                            <div class="form-group">
                                                <label for="ket">Keterangan</label>
                                                <textarea required class="form-control" name="ket" id="ket" rows="2" placeholder="Keterangan"></textarea>
                                            </div>
                                            <button type="submit" name="simpan" class="btn btn-outline-primary mr-2"><i class="fa fa-paper-plane"></i> Submit</button>
                                            <a href="mahasantri.php?id=<?= $id ?>" class="btn btn-outline-warning">Cancel</a>
                                        </form>
                                    </div>
                                </div>
                            </div>
