...
$mhs = mysqli_query($connect, "SELECT * FROM mahasantri WHERE id = $id");
$mh = mysqli_fetch_assoc($mhs);
$mentors = mysqli_query($connect, "SELECT * FROM user WHERE role = 'm'");
if (isset($_POST["sert"])) {
    $nama = htmlspecialchars($_POST["nama"]);
    $nim = htmlspecialchars($_POST["nim"]);
    $mentor = $_POST["mentor"];
    $query = "UPDATE mahasantri SET nama_mhs='$nama', nim = '$nim',mentor_id='$mentor' WHERE id=$id";
                    ...
                    <div class="row">
                        <div class="pt-0 pb-0 p-3 w-100 mt-4">
                            <form method="POST" enctype="multipart/form-data">
                                <h4 class="mb-3">Edit Data <?= $mh['nama_mhs'] ?></h4>
                                <div class="mb-3">
                                    <label for="exampleInputPassword1" class="form-label">Nama Mahasantri</label>
                                    <input type="text" class="form-control" id="" name="nama" value="<?= $mh["nama_mhs"] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="exampleInputPassword1" class="form-label">NIM</label>
                                    <input type="number" class="form-control" id="" name="nim" value="<?= $mh["nim"] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="exampleInputPassword1" class="form-label">Mentor</label>
                                    <select name="mentor" id="" class="form-control">
                                        <option hidden></option>
                                        <?php foreach ($mentors as $mentor) { ?>
                                            <option value="<?= $mentor["id"] ?>" <?php echo ($mentor["id"] == $mh["mentor_id"]) ? 'selected' : ''; ?>>
                                                <?= $mentor["nama"] ?>
                                            </option>
                                        <?php } ?>
                                    </select>
