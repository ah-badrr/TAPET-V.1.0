...
$mentor = mysqli_query($connect, "SELECT * FROM user WHERE role = 'a' AND id = $ide");
$men = mysqli_fetch_assoc($mentor);
if (isset($_POST["sert"])) {
    ...
    // $gambar_dest = "../img/" . $gambar;
    // move_uploaded_file($gambar_temp, $gambar_dest);
    mysqli_query($connect, "UPDATE user SET nama='$nama', username='$username', password='$password' WHERE id = $ide");
    session_start();
    $_SESSION["eu"] = "";
    header("location: setting.php?id=$ide");
    // echo "<script>document.location.href='edit_mentor.php?id=$id&&ide=$ide'</script>";
}
?>
...
<body>
    <div class="container-scroller">
        ...
        text: 'Edit Data Mentor Berhasil!',
        icon: 'success',
        button: 'ok',
        });
        </script>";
                            ...
                            ?>
                                <form method="POST" enctype="multipart/form-data">
                                    <div class="form-group mb-4">
                                        <div class="form-group mb-4">
                                            <label class="text-center w-100">
                                                <div class="poto mb-3" style="background-image:url(../img/<?= $men['gambar'] ?>);"></div>
                                            </label>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="nama" value="<?= $men["nama"] ?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="username" class="form-label">Username</label>
                                        <input type="text" class="form-control" name="username" value="<?= $men["username"] ?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="password" class="form-label">Password</label>
                                        <input type="password" class="form-control" name="password" value="<?= $men["password"] ?>">
                                    </div>
...
if (isset($_POST['poto'])) {
    ...
    $query = "UPDATE user set gambar = '$gambar' WHERE id = $ide";
    mysqli_query($connect, $query);
    session_start();
    $_SESSION["poto"] = "";
    echo "<script>document.location.href='setting.php?id=$ide'</script>";
    // echo "<script>document.location.href='admin.php?id=$ide'</script>";
}
