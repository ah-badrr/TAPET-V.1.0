...
$mtr = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM user WHERE id = $id"));
$mentor = mysqli_query($connect, "SELECT * FROM user WHERE role = 'm'");
if (isset($_POST["sert"])) {
    $nim = htmlspecialchars($_POST["nim"]);
    $nama = htmlspecialchars($_POST["nama"]);

    mysqli_query($connect, "INSERT INTO mahasantri (nim, nama_mhs, mentor_id) VALUES ('$nim', '$nama', '$id')");
    session_start();
    $_SESSION["tmh"] = "";
    header("location: mentor.php?id=$id&&ide=$ide");
}
?>
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    ...
                    <div class="pt-0 pb-0  w-100 mt-4">
                        <h4 class="mb-3">Tambah Data Mahasantri Binaan <?= $mtr['nama'] ?></h4>
                        <form method="POST" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label for="nim" class="form-label">NIM</label>
                                <input type="number" class="form-control" id="nim" aria-describedby="" name="nim">
                            </div>
                            <div class="mb-3">
                                <label for="nama" class="form-label">Nama Mahasantri</label>
                        ...
                        <a href="mentor.php?id=<?= $id ?>&&ide=<?= $ide ?>" class="btn btn-outline-danger"><i class="fa-solid fa-right-from-bracket"></i> Cancel</a>
                    </div>
                </div>
            </div>
        </div>
